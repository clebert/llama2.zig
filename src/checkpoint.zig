...
pub const Checkpoint = struct {
    ...
    fn init(self: *Self) void {
        ...
        const token_embedding = readFloatSlice(&weights_data, self.vocab_size * self.dim);

        const head_size = self.dim / self.n_heads;
        const attention_input_rms = readFloatSlice(&weights_data, self.n_layers * self.dim);
        const attention_query = readFloatSlice(&weights_data, self.n_layers * self.dim * (self.n_heads * head_size));
        const attention_key = readFloatSlice(&weights_data, self.n_layers * self.dim * (self.n_kv_heads * head_size));
        const attention_value = readFloatSlice(&weights_data, self.n_layers * self.dim * (self.n_kv_heads * head_size));
        const attention_output = readFloatSlice(&weights_data, self.n_layers * (self.n_heads * head_size) * self.dim);

        const ffn_input_rms = readFloatSlice(&weights_data, self.n_layers * self.dim);
        const ffn_input_to_hidden = readFloatSlice(&weights_data, self.n_layers * self.dim * self.hidden_dim);
        const ffn_hidden_to_output = readFloatSlice(&weights_data, self.n_layers * self.hidden_dim * self.dim);
        const ffn_input_to_residual = readFloatSlice(&weights_data, self.n_layers * self.dim * self.hidden_dim);

        const final_rms = readFloatSlice(&weights_data, self.dim);
        ...
        self.weights = .{
            ...
            .classifier = if (signed_vocab_size > 0)
                ...
                readFloatSlice(&weights_data, self.vocab_size * self.dim),
        };
    }
